...
Converts .m4a audio files to text using local CPU-based Whisper model
"""
...
try:
    ...
    print("Error: whisper library not installed. Please run: pip install openai-whisper")
    sys.exit(1)
...
def convert_m4a_to_wav(m4a_path, wav_path=None):
    ...
    try:
        with tqdm(total=100, desc="Converting", unit="%", 
...
def transcribe_audio(audio_path, model_size="base", language=None, duration=None):
    ...
    if estimated_time:
        ...
        with tqdm(total=100, desc=desc, unit="%", 
...
def process_audio_file(input_path, output_path=None, model_size="base", language=None, keep_wav=False):
    """
    ...
    if input_path.suffix.lower() != '.m4a':
        print(f"Warning: File is not .m4a format: {input_path.suffix}")
    
    # Try to use Whisper directly on M4A first (it supports many formats)
    # Only convert to WAV if direct processing fails
    audio_path = input_path
    wav_path = None
    needs_conversion = False
    
    if input_path.suffix.lower() == '.m4a':
        print(f"Attempting to process {input_path.name} directly with Whisper...")
        print("(Whisper supports M4A format, so conversion may not be needed)")
    ...
    if duration:
        ...
        print("Could not determine audio duration (progress estimation may be less accurate)")
    
    try:
        # Try transcribing directly first (Whisper supports M4A)
        try:
            ...
            if needs_conversion and input_path.suffix.lower() == '.m4a':
                ...
                try:
                    ...
                    print("  - Try checking the file with: ffmpeg -i files/14-11.m4a")
                    raise convert_error
            else:
                raise direct_error
        ...
        if input_path.suffix.lower() == '.m4a' and not keep_wav and wav_path and wav_path.exists():
            wav_path.unlink()
        ...
        if input_path.suffix.lower() == '.m4a' and not keep_wav:
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert .m4a audio files to text using local CPU-based Whisper model"
    )
    parser.add_argument(
        "input",
        help="Path to input .m4a audio file"
    )
    parser.add_argument(
        "-o", "--output",
        help="Path to output text file (optional)"
    )
    parser.add_argument(
        "-m", "--model",
